...
public class RestaurantsControllerTests:IClassFixture<CustomWebApplicationFactory<Program>>
{
    ...
    {
        factory.Server.PreserveExecutionContext = true; // Khắc phục lỗi liên quan đến async
        _client = factory.WithWebHostBuilder(builder =>
        {
            builder.ConfigureTestServices(services =>
            {
                services.Replace(ServiceDescriptor.Scoped<IRestaurantsRepository>(
    ...
    public async Task GetAll_ForValidRequest_ShouldReturns200Ok()
    {
        ...
        //action
        var response = await _client.GetAsync("/api/restaurants?pageNumber=1&pageSize=10");
